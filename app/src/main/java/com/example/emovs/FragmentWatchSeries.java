...
import com.example.emovs.Loader_Tasks.SessionsLoader;

import java.math.BigInteger;
import java.util.ArrayList;

public class FragmentWatchSeries extends Fragment implements LoaderManager.LoaderCallbacks<Object> , GeneralShowAdapter.ListItemClickListener{
    ...
    com.example.emovs.GeneralShow show = null;

    private static final String SERVER_LOAD_SESSIONS_API = MainActivity.SERVER_ROOT_LINK_URL+"/shows/series";

    public FragmentWatchSeries(String id) {
        show = com.example.emovs.Inventory.getInstance().getShow(new BigInteger(id));
    }
    ...
    private void getSessionsFromServer(String tv_showId) {
        ...
        queryBundle.putString(MainActivity.SERVER_LINK , SERVER_LOAD_SESSIONS_API+"/"+tv_showId);
    ...
    public void onLoadFinished(@NonNull Loader<Object> loader, Object data) {
        ...
        if(id == MainActivity.LOADER_FOR_LOAD_SESSIONS){
            if (data != null) {
                ...
                ArrayList<com.example.emovs.GeneralShow> sessions = new ArrayList<>();
                if(ret.size() ==1) {
                    sessions.add(new com.example.emovs.GeneralShow(new BigInteger(String.valueOf(ret.get(0).getId())), 2, getString(R.string.episodes), show.getPosterImagePath(), "non", null)); }
                else{
                    for (int i = 0; i < ret.size(); i++)
                        sessions.add(new com.example.emovs.GeneralShow(new BigInteger(String.valueOf(ret.get(i).getId())), 2, getString(R.string.session)+": " +String.valueOf(ret.get(i).getSession_number()), show.getPosterImagePath(), "non", null));
                }
    ...
    @Override
    public void onListItemClick(com.example.emovs.GeneralShow pressed_show, String rvDefiningString) {
